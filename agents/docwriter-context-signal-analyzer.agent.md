...
### Signal B1b: Gap pattern extraction (→ candidate gap-patterns for cross-run persistence)

From `gap-analysis.json` + `change-inventory.json` + `impact-matrix.json`:

For EACH gap in `gap-analysis.json`, extract a reusable gap pattern:
1. **Root cause classification**: Why did this gap occur?
   - `missing-cross-ref` — Cross-reference to affected page was missing
   - `stale-content` — Existing content contradicted by changes
   - `undocumented-behavior` — New user-facing behavior with no doc coverage
   - `incomplete-coverage` — Task exists but content doesn't cover all aspects
   - `scope-miss` — Change area wasn't in any task's scope
   - `encoding-violation` — Content formatting / encoding issues
2. **Change domain**: From `change-inventory.json`, which `areaId`(s) produced this gap?
3. **Impact type**: From `impact-matrix.json`, what was the impact type (update/new-page/stale) for the affected page?
4. **Gap predictor**: Formulate a reusable predictor — "When [change domain pattern] changes, [gap type] tends to occur because [root cause]"

Emit one `gapPattern` signal per gap. These accumulate across runs in `meta/gap-patterns/` — the gap-hunter reads them to proactively check known blind spots.
...
### Signal B6: Impact pattern extraction (→ candidate impact-patterns for cross-run persistence)

From `impact-matrix.json` + `change-inventory.json`:

Identify reusable **area→documentation mappings** that downstream impact-mappers can use as warm-start knowledge:

1. For each impact in `impact-matrix.json` where `type` is `update` or `stale`:
   - Record the mapping: `sourceAreaId` → `targetPage` (or its `topicCluster` from doc-index)
   - Record the `impactType` and `priority`
2. Group impacts by area to find patterns:
   - "When [area pattern] changes, pages in [cluster] always need [impact type]"
   - "[area pattern] changes consistently produce [priority] impacts"
3. Abstract area names to patterns where possible:
   - Specific: "AREA-membership-roles" → Abstract: "authentication/membership areas"
   - Keep both the specific area ID and the abstracted pattern

Emit one `impactPattern` signal per unique area→cluster mapping. These accumulate in `meta/impact-patterns/` so the impact-mapper doesn't rediscover the same mappings each run.

**Signal strength**: Single run observation = `low`. If an existing `meta/impact-patterns/` entry matches = `confirm-existing`.
  ...
  "gapPatterns": [
    {
      ...
      "sourceAreaIds": ["AREA-003"],
      "impactType": "update",
      "predictor": "When membership role APIs change, cross-references from secure-pages and member-roles pages are frequently missed",
      "affectedPage": "developers-and-admins/development/registration-and-authentication/member-roles.md",
      "strength": "low"
    }
  ],
  "impactPatterns": [
    {
      ...
      "targetPages": ["business-users/website-content/secure-pages.md"],
      "impactType": "update",
      "priority": "high",
      "strength": "low"
    }
