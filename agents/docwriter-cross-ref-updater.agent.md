...
- `.docwriter/doc-index.json` — cross-reference lookups: `python3 .docwriter/scripts/query-doc-index.py --refs-to "<page-path>" 2>/dev/null` (NEVER read full file)
- `.docwriter/task-graph.json` — completed tasks: `python3 .docwriter/scripts/query-task-graph.py --status written 2>/dev/null`
- `.docwriter/invariant-inventory.json` — link formatting rules: `python3 .docwriter/scripts/query-invariants.py --domain crossref 2>/dev/null`
...
2. **For each affected page, find referring pages.** Using `python3 .docwriter/scripts/query-doc-index.py --refs-to "<affected-page-path>" 2>/dev/null`, find all pages with links TO the affected page. These are candidates for cross-ref updates.
