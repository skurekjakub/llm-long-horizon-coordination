...
Read `.github/skills/docwriter-data-access/SKILL.md` for query script commands. Use these scripts instead of reading full JSON files:
- **Task reads**: `python3 .docwriter/scripts/query-task-graph.py --task-id T-xxx` or `--status planned` or `--summary`
- **Task status updates**: `python3 .docwriter/scripts/update-task-status.py --task-id T-xxx --status written` (surgical — no full-file rewrite)
- **Pipeline overview**: `python3 .docwriter/scripts/query-pipeline-status.py --tasks`
...
3. Reset ONLY those tasks' `status` to `"planned"` using `python3 .docwriter/scripts/update-task-status.py --task-id <id> --status planned` for each affected task. All other `"written"` tasks are preserved — do NOT reset them.
...
Run `python3 .docwriter/scripts/query-task-graph.py --status planned 2>/dev/null` to get the work queue. Read `.docwriter/risk-register.json` for per-task risk scores and mitigations. Process tasks in `order` sequence, respecting `dependsOn` constraints.
...
Update the task status: `python3 .docwriter/scripts/update-task-status.py --task-id <task-id> --status in-progress`
...
- If ALL three reviewers return `"approved"` or `"approved-with-notes"` → task is **accepted**
  - Update task status: `python3 .docwriter/scripts/update-task-status.py --task-id <task-id> --status written`
  - Move to next task
...
- If ANY reviewer returns `"rejected"` → task needs **rewrite**
  ...
  - If attempt >= 3 → mark task as blocked: `python3 .docwriter/scripts/update-task-status.py --task-id <task-id> --status blocked`, log the unresolved issues, move to next task
