...
- `.docwriter/task-graph.json` — completed tasks with target files: `python3 .docwriter/scripts/query-task-graph.py --status written 2>/dev/null`
- `.docwriter/invariant-inventory.json` — Jekyll invariants: `python3 .docwriter/scripts/query-invariants.py --domain jekyll 2>/dev/null` and taxonomy: `python3 .docwriter/scripts/query-invariants.py --domain taxonomy 2>/dev/null`
- `.docwriter/doc-index.json` — valid values: use `python3 .docwriter/scripts/query-doc-index.py` with filters (NEVER read full file)
