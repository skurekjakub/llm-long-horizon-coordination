...
- `.docwriter/meta/impact-patterns/` — persistent area→documentation-cluster mappings from prior runs (IMP-NNN entries)
...
1. **For each area in change-inventory**, cross-reference with doc-index topic clusters:
   ...
   - **Warm-start from impact patterns**: If `meta/impact-patterns/` exists, read IMP-NNN entries. For each pattern where `sourceAreaPattern` matches the current area (by keyword overlap or direct area ID match), use the pattern's `targetClusters` and `targetPages` as initial candidates. This avoids rediscovering the same area→cluster mappings from scratch each run. Still verify each candidate against the actual change — patterns are starting points, not guaranteed matches.
