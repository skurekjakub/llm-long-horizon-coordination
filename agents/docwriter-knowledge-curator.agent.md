...
- `.docwriter/meta/gap-patterns/*.md` — persistent gap root-cause patterns (which change domains produce which gap types)
- `.docwriter/meta/impact-patterns/*.md` — persistent area→documentation-cluster mappings
...
### Step 2b: TTL-based staleness enforcement

Before scoring entries, apply staleness enforcement to the index:

For each non-deprecated entry where `shouldReviewAfter` exists and `shouldReviewAfter < now`:
- If `lastReferencedDate < shouldReviewAfter` (entry was not re-referenced since TTL):
  - Flag the entry as `stale` in your curation log
  - If the entry's `effectiveConfidence` has already been decayed by the integrator → use the decayed `effectiveConfidence` for scoring, not the original `confidence`
  - If `effectiveConfidence` is absent (legacy entry), compute it: apply one step decay if past TTL

**This step does NOT modify index.json.** The curator is read-only. Decay writes are the knowledge-integrator's responsibility. The curator respects whatever `effectiveConfidence` the integrator has set.
...
| **Confidence** | 25% | Use `effectiveConfidence` if present, otherwise `confidence`. `high` = 1.0, `medium` = 0.6, `low` = 0.3 |
...
| `gap-pattern` | gap-hunter (proactive blind-spot checking) | P2 — read directly by gap-hunter from meta/gap-patterns/ |
| `impact-pattern` | impact-mapper (warm-start area→cluster mapping) | P2 — read directly by impact-mapper from meta/impact-patterns/ |
