...
- `.docwriter/doc-index.json` — corpus map: use `python3 .docwriter/scripts/query-doc-index.py` with targeted queries (see skill below)
- `.docwriter/invariant-inventory.json` — indexed invariants (full read required for inlining into tasks)
- `.docwriter/code-analysis.json` — behavioral facts: `python3 .docwriter/scripts/query-code-analysis.py --summary 2>/dev/null` for overview, then `--area` for specifics

See `.github/skills/docwriter-data-access/SKILL.md` for the full query script reference.

**Note:** You generate task-graph.json — write logic stays as-is. Query scripts are for READ optimization on inputs.
